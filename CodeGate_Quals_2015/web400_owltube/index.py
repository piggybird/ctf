...
@app.before_request
def before_request():
	g.db = pymongo.Connection("mongodb://128.199.123.177").owltube
	g.user = None
...
def get_cookie():
	...
	app.logger.debug('GetCookie' + cookie + "len:" + str(len(cookie)))
	iv = cookie[:BS]
	cookie = cookie[BS:]
	...
	app.logger.debug('GetCookieAfterDecrypt' + cookie + "len:" + str(len(cookie)))
...
def set_cookie(resp, cookie):
	...
	app.logger.debug('SetCookie' + cookie + "len:" + str(len(cookie)))

	iv = Random.new().read(BS)
	aes = AES.new(SECRET_KEY, AES.MODE_CBC, iv)
	cookie = pad(cookie)
	cookie = iv + aes.encrypt(cookie)
	app.logger.debug('SetCookieAfterEncrypt' + cookie + "len:" + str(len(cookie)))
...
def is_logged_in():
	...
	app.logger.debug('QueryUser:' + str(cookie))
...
def index():
	if is_logged_in():
		videos = []
		app.logger.debug('QueryVideo:' + str({"user": g.user["u"]}));
...
def login():
	...
	app.logger.debug('postvalue' + str(u))
...
def addvid():
	...
	app.logger.debug('InsertVIDQuery:' + str(vid))
	g.db.videos.insert(vid)
